...
| phase | string | 否 | 分片阶段，推荐值：`chunk` / `snapshot` / `final` |
| content_type | string | 否 | 内容语义：`delta`（追加增量）/ `snapshot`（整句快照） |
| reason | string | 否 | 结束原因（仅 `done=true` 时建议携带）：`complete` / `error` |

流式建议：

1. 追加增量分片：`done=false`，`phase=chunk`，建议同时携带 `content_type=delta`，`content` 为新增文本  
2. 若出现整句重写/快照：`done=false`，应使用 `phase=snapshot`，并建议携带 `content_type=snapshot`；此时 `content` 表示当前完整文本快照，而不是追加片段  
3. 结束分片：`done=true`，`phase=final`，建议携带 `reason`  
4. 若结束分片仅用于通知结束，可发送空 `content`（`""`）  
5. 接收端以 `done=true` 作为一轮回复结束标志，而不是依赖超时或连接关闭
...
- 若收到 `phase=snapshot`，应以该帧 `content` 覆盖当前已累计文本，而不是继续追加
...
function onResponse(msg: any) {
  ...
  if (content) {
    if (md.phase === "snapshot") {
      st.text = content;
      handleSnapshotText(cid, content);
    } else {
      st.text += content;
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.4.0 | 2026-03-27 | 明确区分流式增量分片与整句快照分片，新增 `phase=snapshot` / `content_type` 约定 |
